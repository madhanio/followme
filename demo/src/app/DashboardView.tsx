...
export default function DashboardView({ initialRepos, initialLogs, initialRunSummary = [] }: DashboardViewProps) {
  ...
  const defaultSettings = useMemo(() => ({
    ...
    recipientEmail: 'admin@followme.demo',
    digestSummary: {
      runSummary: true,
  ...
  const handleSync = async () => {
    ...
    setTimeout(() => {
      setLogs(prev => [{
        ...
        status: 'SUCCESS',
        message: '[Demo] Synced following list with GitHub API.'
      }, ...prev]);
      setIsSyncing(false);
    }, 400);
  };
  ...
  const handleCleanupRun = async () => {
    ...
    setTimeout(() => {
      setCleanupStatus({ success: true, message: 'Demo Mode: Stale records cleanup simulated.' });
      setIsCleaning(false);
  ...
  const handleLogCleanupRun = async () => {
    ...
    setTimeout(() => {
      setLogs([]);
      setCleanupStatus({ success: true, message: 'Demo Mode: Audit logs cleared.' });
      setIsCleaning(false);
      setIsRefreshing(false);
    }, 400);
  };
  ...
  const handleClearStaleRun = async () => {
    ...
    setIsRefreshing(true);
    setTimeout(() => {
      setRepos(prev => prev.filter(r => !r.unfollowed));
      setCleanupStatus({ success: true, message: 'Demo Mode: Cleared stale profiles from view.' });
      setIsCleaning(false);
      setIsRefreshing(false);
    }, 400);
  };
  ...
  const handleUpdateSecurityKey = async (e: React.FormEvent) => {
    e.preventDefault();
    setSecKeyError('Password updates are disabled in public Demo Mode.');
  ...
  const handleTrigger = async () => {
    ...
    setTimeout(() => {
      setTriggerStatus({ success: true, message: 'Demo Mode: Candidate search & AI evaluation job simulated.' });
  ...
  const handleFollowUser = async (username: string) => {
    ...
    setLogs(prev => [{
      ...
      timestamp: new Date().toISOString(),
      status: 'SUCCESS',
      message: `Followed target user: ${username}`
    }, ...prev]);
  ...
  const handleStar = async (owner: string, name: string) => {
    ...
    setLogs(prev => [{
      ...
      timestamp: new Date().toISOString(),
      status: 'SUCCESS',
      message: `Starred repository ${owner}/${name}`
    }, ...prev]);
  };
  ...
  const handleUnstar = async (owner: string, name: string) => {
    ...
    setLogs(prev => [{
      ...
      timestamp: new Date().toISOString(),
      status: 'SUCCESS',
      message: `Unstarred repository ${owner}/${name}`
    }, ...prev]);
  };

  const handleDeleteProfile = async (username: string) => {
    if (!confirm(`Are you sure you want to permanently delete @${username} and all of their repositories from the view?`)) {
      return;
    }
    setIsActionLoading(true);
    ...
    setLogs(prev => [{
      ...
      timestamp: new Date().toISOString(),
      status: 'SUCCESS',
      message: `Deleted developer profile @${username}`
    }, ...prev]);
    setIsActionLoading(false);
  };
                      ...
                      <div className="bg-[#09090b] text-zinc-300 font-mono text-xs p-5 overflow-y-auto no-scrollbar h-[480px] space-y-3.5 rounded-b-2xl border border-zinc-800 border-t-0 select-text">
                        {isRefreshing ? (
                          ...
                          if (displayedLogs.length === 0) {
                            return (
                              <div className="py-12 text-center text-zinc-500 space-y-3 flex flex-col items-center justify-center">
                                <div>No active logs matching search filters or logs were cleared.</div>
                                <button 
                                  onClick={() => setLogs(initialLogs)}
                                  className="px-4 py-1.5 bg-[#e60023] hover:bg-[#c0001b] text-white text-xs font-bold rounded-full transition-all cursor-pointer shadow-sm active:scale-95 inline-flex items-center space-x-1.5"
                                >
